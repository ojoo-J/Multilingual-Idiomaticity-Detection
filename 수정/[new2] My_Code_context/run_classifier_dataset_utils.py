...
def convert_examples_to_two_features_with_context(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        tokens_mwe = tokenizer.tokenize(example.mwe)
        tokens_all = tokens_prev + tokens_a + tokens_next
        tokens_third = tokenizer.tokenize(example.text_a)
        tokens_b = None
        mwe_index = None
        ...
        if (len(tokens_all)+len(tokens_mwe)) > max_seq_length - 6:
                tokens_all = tokens_all[: (max_seq_length - 6 - len(tokens_mwe))]
        

        tokens = [tokenizer.cls_token] + tokens_all + tokens_mwe + [tokenizer.sep_token]
        #print('tokens:', tokens)
        # POS tag tokens
        if False:
            POS_token = tokenizer.tokenize(example.POS)
        ...
        if index_yes==True:
            tokens_b += 1  # add 1 to the target word index considering [CLS]
            try:
                for i in range(len(tokens_mwe)): # text_b -> text_b_list
                    segment_ids[tokens_b + i] = 1
            except:
                pass
        ...
        if len(tokens_third) > max_seq_length - 6:
            tokens_third = tokens_third[: (max_seq_length - 6)]

        tokens = [tokenizer.cls_token] + tokens_third + [tokenizer.sep_token]

        segment_ids_3 = [0] * len(tokens)

        if index_yes==True:
            tokens_b_ += 1  # add 1 to the target word index considering [CLS]
            try:
                for i in range(len(sum(text_b_list, []))):
                    segment_ids_3[tokens_b_ + i] = 1
            except:
                pass
        ...
        #print("mwe: ", sum(text_b_list, []))
        #print("third tokens: ", tokens)
        #print("segment_ids_3: ", segment_ids_3)
