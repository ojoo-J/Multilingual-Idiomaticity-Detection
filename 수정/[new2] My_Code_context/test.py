...
a = ['<s>', '▁"', 'Se', 'gu', 'ndo', '▁a', '▁fonte', ',', '▁o', '▁programa', '▁vai', '▁seguir', '▁um', '▁formato', '▁no', '▁mesmo', '▁estilo', '▁do', '▁antigo', '▁programa', '▁de', '▁Cha', 'c', 'rinha', ':', '▁"', '"', 'Os', '▁cara', 's', '▁estão', '▁monta', 'ndo', '▁uma', '▁equipe', '▁pro', '▁Gerald', 'o', ',', '▁estão', '▁monta', 'ndo', '▁tudo', '▁lá', ',', '▁vão', '▁colocar', '▁lá', '▁um', '▁monte', '▁de']
print(len(a))
...
def combine_zero_one():
    ...
    zero = pd.read_csv('/home/ojoo/SemEval-2022-Task2/MelBERT/[new2] My_Code_context/zero.csv')
    one = pd.read_csv('/home/ojoo/SemEval-2022-Task2/MelBERT/[new2] My_Code_context/one.csv')
    one['Setting'] = 'one_shot'
    len(zero)
    len(one)
    submission = pd.concat([zero, one], axis=0)
    len(submission)
    submission.to_csv('/home/ojoo/SemEval-2022-Task2/MelBERT/[new2] My_Code_context/task2_subtaska.csv', index=False, encoding='cp949')
