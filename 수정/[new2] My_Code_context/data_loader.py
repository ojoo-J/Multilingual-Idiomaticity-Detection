...
from run_classifier_dataset_utils import (
    ...
    convert_examples_to_two_features_with_context,
    convert_two_examples_to_features,
    convert_examples_to_features_with_context
)
...
def load_train_data(args, logger, processor, task_name, label_list, tokenizer, output_mode, k=None):
    ...
    if task_name == "idiom":
        ...
        raise ("task_name should be 'idiom'!")
    ...
    if args.model_type == "MELBERT_SPV_CONTEXT":
        train_features = convert_examples_to_features_with_context(
            train_examples, label_list, args.max_seq_length, tokenizer, output_mode, args
        )
    ...
    if args.model_type in ["MELBERT_CONTEXT"]:
...
def load_dev_data(args, logger, processor, task_name, label_list, tokenizer, output_mode, k=None):
    ...
    if args.model_type == "MELBERT_SPV_CONTEXT":
        dev_features = convert_examples_to_features_with_context(
            dev_examples, label_list, args.max_seq_length, tokenizer, output_mode, args
        )
    if args.model_type in ["MELBERT_MIP", "MELBERT"]:
...
def load_eval_data(args, logger, processor, task_name, label_list, tokenizer, output_mode, k=None):
    if task_name == "idiom":
        ...
        eval_examples = processor.get_eval_examples(args.data_dir, k)
    else:
        raise ("task_name should be 'idiom'!")
    ...
    if args.model_type == "MELBERT_SPV_CONTEXT":
        eval_features = convert_examples_to_features_with_context(
            eval_examples, label_list, args.max_seq_length, tokenizer, output_mode, args
        )
    if args.model_type in ["MELBERT_MIP", "MELBERT"]:
