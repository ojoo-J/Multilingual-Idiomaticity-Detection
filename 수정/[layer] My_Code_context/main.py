...
from data_loader import load_train_data, load_train_data_kf, load_eval_data, load_dev_data
...
def main():
    ...
    '''
    # idiom - dev
    if args.do_dev and task_name == "idiom":
        all_guids, dev_dataloader = load_dev_data(
            args, logger, processor, task_name, label_list, tokenizer, output_mode
        )

        run_dev(args, logger, model, dev_dataloader, all_guids, task_name, return_preds=False)
    '''
    ...
    if args.do_eval and task_name == "idiom":
        ...
        if ("genre" in args.data_dir) or ("pos" in args.data_dir):
            ...
            for idx, target in tqdm(enumerate(targets)):
                ...
                run_dev(args, logger, model, eval_dataloader, all_guids, task_name, return_preds=True)
        else:
            all_guids, eval_dataloader = load_eval_data(
                args, logger, processor, task_name, label_list, tokenizer, output_mode
            )
    ...
    # TroFi / MOH-X (K-fold)
    elif args.do_eval and args.task_name == "trofi":
        ...
        for k in tqdm(range(10), desc="K-fold"):
            ...
            result = run_dev(args, logger, model, eval_dataloader, all_guids, task_name)
    ...
    for epoch in trange(int(args.num_train_epoch), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
            ...
            if args.model_type in ["BERT_SEQ", "BERT_BASE", "MELBERT_SPV", "MELBERT_SPV_CONTEXT"]:
                ...
                loss_fct = nn.NLLLoss(weight=torch.Tensor([1, args.class_weight]).to(args.device)) # weight=torch.Tensor([1, args.class_weight]).to(args.device)
        ...
        if args.do_dev:
            ...
            result_list.append(result["f1_macro"])
