...
# melbert_spv, melbert_spv_context
class AutoModelForSequenceClassification_SPV(nn.Module):
    ...
    def __init__(self, args, Model, config, num_labels=2):
        ...
        self.config = config
        self.dropout = nn.Dropout(args.drop_ratio)
        self.SPV_linear = nn.Linear(config.hidden_size * 2, args.classifier_hidden) # SPV layer 추가
        self.classifier = nn.Linear(args.classifier_hidden, num_labels)
        self.logsoftmax = nn.LogSoftmax(dim=1)
    ...
    def forward(
        ...
        logits = self.classifier(SPV_hidden)
        logits = self.logsoftmax(logits)
...
class AutoModelForSequenceClassification_SPV_MIP(nn.Module):
    ...
    def __init__(self, args, Model, config, num_labels=2):
        ...
        self.dropout = nn.Dropout(args.drop_ratio)
        self.args = args
        #self.static = nn.Embedding(num_embeddings=250002,embedding_dim=config.hidden_size)
        #self.sent_targ = nn.Linear(config.hidden_size * 2, args.classifier_hidden)
        #self.sent_targ2 = nn.Linear(config.hidden_size * 2, args.classifier_hidden)
        #self.targ_targ2 = nn.Linear(config.hidden_size * 2, args.classifier_hidden)
        self.classifier = nn.Linear(args.classifier_hidden * 3, num_labels)
        #self._init_weights(self.sent_targ)
        #self._init_weights(self.sent_targ2)
        #self._init_weights(self.targ_targ2)
    ...
    def forward(
        ...
        outputs_2 = self.encoder(input_ids_2, attention_mask=attention_mask_2, head_mask=head_mask)
        sequence_output_2 = outputs_2[0]  # [batch, max_len, hidden]
        #target_output_2 = self.static(input_ids_2)
        ...
        #hidden1 = self.sent_targ(torch.cat([pooled_output, target_output], dim=1))
        #hidden2 = self.sent_targ(torch.cat([pooled_output, target_output_2], dim=1))
        #hidden3 = self.targ_targ2(torch.cat([target_output_2, target_output], dim=1))

        logits = self.classifier(self.dropout(torch.cat([pooled_output, target_output, target_output_2], dim=1)))
        logits = self.logsoftmax(logits)
