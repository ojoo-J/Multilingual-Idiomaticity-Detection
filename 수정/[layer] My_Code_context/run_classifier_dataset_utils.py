...
def convert_examples_to_features_with_context(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # truncate the sentence to max_seq_len
        if (len(tokens_all)+len(tokens_mwe)) > max_seq_length - 2:
            tokens_all = tokens_all[: (max_seq_length - 2 - len(tokens_mwe))]
...
def convert_examples_to_two_features_with_context(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        tokens = [tokenizer.cls_token] + tokens_all + [tokenizer.sep_token] + tokens_mwe + [tokenizer.sep_token]
