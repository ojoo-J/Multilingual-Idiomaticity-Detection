...
class AutoModelForSequenceClassification_SPV_MIP(nn.Module):
    ...
    def forward(
        ...
        contx_hidden = self.hidden_linear_1(torch.cat([pooled_output, mwe_output], dim=1))
        comp_hidden = self.hidden_linear_2(torch.cat([pooled_output_3, mwe_output_2], dim=1))

        logits = self.classifier(self.dropout(torch.cat([contx_hidden, comp_hidden], dim=1)))
